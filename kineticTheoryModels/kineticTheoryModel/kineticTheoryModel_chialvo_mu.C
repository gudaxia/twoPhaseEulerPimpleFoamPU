 ...
 */
 ...
 else if(mofidiedKineticTheoryPU_)
 {
	...
	Theta_ = max(ThetaDil_,ThetaDense_) ;
     ...
     // Frictional viscosity, Chialvo
     forAll (Ua_.mesh().cells(), celli)
     {
        if (alpha_[celli] > alphac_)
        {
            mua_[celli] = pa_[celli] * upsilon_[celli] / max( gammaDot[celli], gammaDotSmall.value() );
        }
     }
     //mua_.min(1.0e+2);
     mua_.max(0.0);
