...
tmp<volScalarField> Foam::kineticTheoryModel::ppMagfCalc(const volScalarField& alphaUpdate)
{
  ...
  (
      Ua_.mesh(),
      min(max(alpha, scalar(constSMALL)), alphac_ - alphad_),
      alphaMax_,
      alphaf_,
      alphac_
  );
 ...
 */
 ...
 else if(mofidiedKineticTheoryPU_)
 {
     ...
     Info<< "kinTheory: min(kappa) = " << min(kappa_).value()
         << ", max(kappa) = "          << max(kappa_).value() << endl;
	...
	Theta_ = max(ThetaDil_,ThetaDense_) ;
     ...
     // Relaxation granular temperature
     Theta_.relax();
     ...
     // YG  Blended model including the intermediate regime.
     pa_ = 1/(1/min(pa_,constSMALL) + 1/(min(0.099*pow(gammaDot,0.5),constSMALL*dim[0 0 -0.5 0 0 0 0])));
     // Relaxation pa_
     pa_.relax();
